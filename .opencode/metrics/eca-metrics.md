...
changelog.md,dcon-report.md,red-team-*-report.md}`, `.opencode/lessons/`,
`.opencode/rules/agents/*`, git history. Prod incident/rollback data source:
...
| **Lessons-applied rate** | % of new features whose `overview.md` §Research references ≥1 prior lesson | feature dirs + `.opencode/lessons/index.md` | computable now |
...
| **Repeat-error rate** | same error recurring across sessions — should trend to **zero** if the loop works; the proof the loop is real | `.claude/metrics/aggregated/rule-effectiveness.json` (`recurrences_after`) via `rule-stats.sh` | computable now |
...
and writes `.claude/metrics/aggregated/eca-metrics.json` (mirroring how
`rule-effectiveness.json` backs the learning loop). The **repeat-error rate** is
reported by the learning loop's own `bash .claude/scripts/rule-stats.sh`.
