---
description: Read-only status check for a feature. Renders the ECA board (tier, activation manifest, per-scenario lifecycle, checkpoint opt-ins) from status.md, regenerates the machine test-state status.yaml, lints scenario blocks, runs scope check, and prints a summary. Safe to run any time.
---
