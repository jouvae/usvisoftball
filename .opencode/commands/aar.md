...
`.opencode/rules/LEARNING-LOOP.md`. **Do not fork a second learning system** — AAR
...
Read `.opencode/skills/aar/SKILL.md` (lesson record schema + tier tagging).

### Step 2 — Write the lesson

Append a tier-tagged lesson row to `docs/features/{group}/{feature}/lessons.md` and a
durable record to `.opencode/lessons/` (the never-deleted audit trail), updating
`.opencode/lessons/index.md`. Tag every lesson with the **active tier** (from
...
- Durable record: .opencode/lessons/{file}
