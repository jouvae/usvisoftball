...
// The roster player delete island (teams-e2e-004). A thin `<form action>` around an
// id-bound delete action (the page binds the id server-side, so the island never carries a
// forgeable id). `useActionState` tracks pending + surfaces the returned error; on success
// the action revalidates this route so the list re-renders in the same response.
