...
**Before working, read the ratified rules that govern your surface** — `.claude/rules/frontend.md` (BFF error-code preservation, concurrent forwards, mock/contract parity, route↔type parity) and `.claude/rules/security.md` §S2 (the anti-enumeration UI invariant). They are binding; there is no provisional tier in the repo any more.
...
3. **`.claude/skills/scope-discipline/SKILL.md`** (if invoked from feature flow) — do not modify files outside the scope passed by the caller.
